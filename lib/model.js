...
function shutBlocks (graph, open) {
  ...
  for (const node of graph.nodes) {
    ...
    if (surface !== undefined && surface.shut === true) {
      ...
      // A block answers to whatever anything inside it answers to, which is what
      // makes it wirable as one thing.
      for (const out of node.outputs || []) {
        if (!surface.outputs.some((/** @type {any} */ o) => o.contract === out.contract)) {
          surface.outputs.push({ ...out })
        }
      }
      continue
    }
