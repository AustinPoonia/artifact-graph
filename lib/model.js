 ...
 * @property {{ contract: string, used: boolean }[]} outputs  the same, as sockets
 *   on the node's right, each saying whether anything on this graph takes it
...
function model ({ instances, manifests, problems = [] } = /** @type {any} */ ({})) {
  ...
  for (const instance of all) {
    ...
    nodes.push({
      ...
      outputs: /** @type {{ contract: string, used: boolean }[]} */ ([]),
