...
function heightOf (node) {
  return NODE.head + rowsOf(node) * NODE.port + NODE.foot
}

/**
 * How many socket rows a node draws: what it takes, then what it gives.
 *
 * Outputs sit *below* the inputs rather than sharing their rows. A node editor
 * usually puts both at the top and lets them face each other, and that works
 * where a node has two of each; here a studio has eleven inputs and one output,
 * so shared rows would put an output label beside an unrelated input label
 * eleven times and beside the right one once.
 *
 * @param {any} node
 */
function rowsOf (node) {
  return (node.ports || []).length + (node.outputs || []).length
}

/**
 * Where a node's output socket sits, by index among its outputs.
 *
 * The same row pitch as an input, continuing past the last one — so both ends of
 * a spline are computed by one function of one set of numbers, and an edge
 * cannot land between two sockets because two formulas disagreed.
 *
 * @param {any} node @param {number} index
 */
function outAt (node, index) {
  const after = (node.ports || []).length + Math.max(0, index)
  return { x: NODE.width, y: NODE.head + after * NODE.port + NODE.port / 2 }
}
...
function layout (graph, saved = {}) {
  ...
  const drawn = edges.map((edge) => {
    ...
    // The provider's *matching* output. A node answering on its whole right side
    // was the first version, and it put every edge into one node at the same
    // point — so a provider of two contracts drew two lines out of one place and
    // the picture stopped saying which was which.
    const gives = (to.outputs || []).findIndex((/** @type {any} */ o) => o.contract === edge.contract)
    const out = outAt(to, gives < 0 ? 0 : gives)
    ...
    return {
      ...
      x1: from.x + socket.x,
      y1: from.y + socket.y,
      // The provider's output dot, on its right edge.
      x2: to.x + out.x,
      y2: to.y + out.y,
...
module.exports = { layout, rankOf, order, heightOf, rowsOf, portAt, outAt, NODE, GAP }
