...
;(function () {
  ...
  document.addEventListener('click', function (e) {
    ...
    if (e.target.closest('[data-aside-close]')) { goTo('canvas'); return }
   ...
   */
  function openAside (open, heading) {
    var rail = document.querySelector('[data-aside]')
    if (!rail) return
    rail.setAttribute('data-open', open ? 'true' : 'false')

    var name = rail.querySelector('[data-aside-title]')
    if (name) name.textContent = open ? (heading || '') : ''
    if (!open) show('node-panel', '')

    // 220ms: the transition is 200. Fitting before it finishes fits to a width
    // the rail is passing through.
    setTimeout(fit, 220)
  }
