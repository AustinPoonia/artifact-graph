...
;(function () {
  ...
  /**
   * Whether an armed socket is one the open block currently surfaces.
   *
   * Read off the drawing rather than kept in a second copy: inside a block, an
   * exposed port is one with an edge to In, and that edge is on the page.
   */
  function onSurface (armed) {
    if (inside === '') return false
    var mark = armed.side === 'out'
      ? inside + '/out ' + armed.contract + ' ' + armed.id
      : armed.id + ' ' + armed.port + ' ' + inside + '/in'
    var all = document.querySelectorAll('[data-edge]')
    for (var i = 0; i < all.length; i++) {
      if (String(all[i].getAttribute('data-edge')).indexOf(mark) === 0) return true
    }
    return false
  }
  ...
  /**
   * A wire that comes out of the socket and follows the pointer.
   *
   * Without it an armed port is a dot that changed colour, and the answer to
   * "what is this gesture doing" is nothing you can see. The curve is the one
   * the layout draws and restring() redraws, because a wire that snapped into a
   * different shape the moment it landed would be two drawings of one edge.
   */
  function livewire (clientX, clientY) {
    if (!arming || !arming.el) return
    var box = viewport()
    var svg = document.querySelector('[data-graph-edges]')
    if (!box || !svg) return

    var line = document.querySelector('[data-graph-live]')
    if (!line) {
      line = document.createElementNS('http://www.w3.org/2000/svg', 'path')
      line.setAttribute('class', 'k-graph-edge k-graph-edge-live')
      line.setAttribute('data-graph-live', 'true')
      line.setAttribute('fill', 'none')
      line.setAttribute('vector-effect', 'non-scaling-stroke')
      svg.appendChild(line)
    }

    // The dot's centre, and the pointer, both in the world the edges are drawn
    // in -- the layer is inside the same transform the nodes are.
    var at = box.getBoundingClientRect()
    var dot = arming.el.querySelector('.k-graph-dot') || arming.el
    var spot = dot.getBoundingClientRect()
    var x1 = (spot.left + spot.width / 2 - at.left - view.x) / view.scale
    var y1 = (spot.top + spot.height / 2 - at.top - view.y) / view.scale
    var x2 = (clientX - at.left - view.x) / view.scale
    var y2 = (clientY - at.top - view.y) / view.scale

    // An input leaves leftwards and an output rightwards, which is the shape the
    // server draws and the reason two edges into one node stay legible.
    var reach = Math.max(40, Math.abs(x1 - x2) / 2)
    var d = arming.side === 'out'
      ? 'M' + x2 + ' ' + y2 + 'C' + (x2 - reach) + ' ' + y2 + ' ' + (x1 + reach) + ' ' + y1 + ' ' + x1 + ' ' + y1
      : 'M' + x1 + ' ' + y1 + 'C' + (x1 - reach) + ' ' + y1 + ' ' + (x2 + reach) + ' ' + y2 + ' ' + x2 + ' ' + y2
    line.setAttribute('d', d)
  }

  /**
   * Take the open node off the drawing.
   *
   * The node the rail is about, because that is the one on screen with a name on
   * it -- a canvas with no selection has nothing to delete, and guessing from
   * the pointer is how somebody loses a node they were only hovering.
   */
  function drop () {
    if (!focused) { announce('Open the node you want taken off, then ask again.'); return }
    var going = focused
    return draft({ do: 'drop', id: going }).then(function (a) {
      if (a && a.ok) { focused = ''; goTo('canvas') }
      return a
    })
  }
  ...
  document.addEventListener('click', function (e) {
    ...
    if (e.target.closest('[data-graph-reset]')) {
      ask('/places', JSON.stringify({})).then(function (a) {
    ...
    // Wiring is on the pointer, above -- a click never sees the port a drag
    // started from.
  ...
  document.addEventListener('pointerdown', function (e) {
    ...
    if (e.target.closest('[data-graph-tool]') || e.target.closest('[data-graph-into]') ||
        e.target.closest('[data-graph-zoom]') || e.target.closest('[data-graph-fit]') ||
        e.target.closest('[data-graph-reset]')) return
    ...
    // Wiring happens on the *pointer*, not on clicks, and that is not a
    // preference. A drag from a port to a node fires its click on the nearest
    // common ancestor -- the world, or the viewport -- so a click handler never
    // sees the port at all, and the gesture everybody tries first did nothing
    // whatsoever. Arming here and landing on the way up serves the drag and the
    // two clicks with one path.
    if (tool === 'wire') {
      if (arming) return
      ...
      if (socket) {
        ...
        livewire(e.clientX, e.clientY)
        announce(inside === ''
          ? 'Now let go on the node that answers ' + said[1] + '.'
          : 'Now let go on the node that answers it, or on In to make it a port of this block.')
        return
      }
      // An output is only wirable inside a block: out there it is something a
      // node *has*, and in here it is something the block can be made to answer to.
      var gives = inside === '' ? null : e.target.closest('[data-out]')
      if (gives) {
        ...
        livewire(e.clientX, e.clientY)
        announce('Now let go on Out to make this block answer to ' + offered[1] + '.')
      }
      return
    }
  ...
  document.addEventListener('pointerup', function (e) {
    ...
    // Landing a wire. Released on the node it started from, it stays armed, so a
    // press-and-release on a port followed by one on the target is the same
    // gesture as dragging between them.
    if (tool === 'wire' && arming) {
      var onto = e && e.target && e.target.closest ? e.target.closest('[data-node]') : null
      var to = onto && onto.getAttribute('data-node')
      // Let go over nothing, having dragged: the wire is being taken off. On the
      // surface that means the port stops being a port of the block, and
      // anywhere else it means the binding goes -- which is the same gesture
      // meaning the same thing in both places.
      if (!to && arming.moved) {
        var dropped = arming
        disarmWire()
        if (inside !== '' && onSurface(dropped)) {
          draft({
            do: 'expose',
            block: inside,
            role: roleOf(dropped.id),
            side: dropped.side === 'out' ? 'out' : 'in',
            port: dropped.port || '',
            contract: dropped.contract || ''
          })
          return
        }
        if (dropped.side === 'out') { announce('An output is not bound to anything, so there is nothing to take off.'); return }
        draft({ do: 'unwire', from: dropped.id, port: dropped.port })
        return
      }
  ...
  document.addEventListener('keydown', function (e) {
    ...
    else if ((e.key === 'Delete' || e.key === 'Backspace') && focused) { drop(); e.preventDefault() }
    else if (e.key === 'Enter') {
      var node = e.target.closest('[data-node]')
  ...
  document.addEventListener('pointermove', function (e) {
    ...
    if (arming) { arming.moved = true; livewire(e.clientX, e.clientY) }
  })
