...
;(function () {
  ...
  document.addEventListener('click', function (e) {
    ...
    if (into) {
      var block = into.getAttribute('data-graph-into')
      draft({ do: into.getAttribute('aria-expanded') === 'true' ? 'shut' : 'open', block: block })
      return
    }
    ...
    if (tool === 'wire') {
      ...
      if (socket && !arming) {
        ...
        disarmWire()
        arming = { el: socket, id: said[0], port: said[1] }
        socket.setAttribute('data-arm', 'from')
        announce('Now click the node that answers ' + said[1] + '.')
        return
      }
