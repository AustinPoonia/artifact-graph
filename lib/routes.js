...
function routes ({ kit, script, store, title = 'Graph' }) {
  ...
  function cut (sent) {
    ...
    // `into`, not `block`. One field meaning "the block this names" and another
    // meaning "the canvas we are inside" is one field too few: placing a node
    // while inside a block read `block` as the former and put the node on the
    // main canvas, which is exactly what it says and not at all what was meant.
    const inside = text(sent.into) === '' ? null : insideOf(text(sent.into))
    if (inside) {
      return {
        graph: inside,
        said: [`inside ${text(sent.into)}`],
        over: 0,
    ...
    async handle (req) {
      ...
      const wire = (one, port, to) => {
        ...
        if (act === 'block') {
          ...
          const into = text(sent.into)
          const host = into === '' ? null : drafts.find((d) => d.block === into && d.expose !== undefined)
          ...
          for (const part of worked.parts) {
            ...
            drafts.push({
              ...
              bindings,
              draft: true,
              // Inside a block, a placed block's parts join the block that is
              // open rather than starting one of their own: a block inside a
              // block is a concept this platform does not have, and pretending
              // otherwise would draw a node nothing can sign.
              block: host ? into : block,
              role: host ? /** @type {string} */ (byRole.get(part.role)) : part.role,
        ...
        // Taking a placed node away. Only a draft: a signed instance is removed by
        // `instance remove`, which burns its id, and that is a command an
        // administrator runs rather than a key somebody presses.
        if (act === 'drop') {
          const id = text(sent.id)
          const one = drafts.find((d) => d.id === id)
          if (!one) {
            const exists = signed().nodes.some((n) => n.id === id)
            return json(200, {
              ok: false,
              mode: chosen,
              signed: exists,
              why: exists
                ? `${id} is signed, so it cannot be taken off a drawing. Removing it burns the id — open it ` +
                  'and work the removal out.'
                : `there is nothing placed called ${id}`
            })
          }

          // A whole block goes together. Dropping the part that carries the
          // surface and leaving the rest would leave instances nothing names and
          // nothing can reach.
          const going = one.block !== undefined && one.expose !== undefined
            ? drafts.filter((d) => d.block === one.block).map((d) => d.id)
            : [id]
          drafts = drafts.filter((d) => !going.includes(d.id))

          // And nothing may still be bound to what has gone.
          for (const rest of drafts) {
            for (const [name, bound] of Object.entries(rest.bindings)) {
              if (Array.isArray(bound)) {
                rest.bindings[name] = bound.filter((/** @type {string} */ t) => !going.includes(t))
              } else if (typeof bound === 'string' && going.includes(bound)) {
                rest.bindings[name] = null
              }
            }
          }
          ...
          const out = /** @type {any} */ (JSON.parse(answer().body))
          out.dropped = going
          out.why = going.length === 1
            ? `${id} is off the drawing.`
            : `${id} and ${going.length - 1} more are off the drawing.`
          return { status: 200, type: 'application/json; charset=utf-8', body: JSON.stringify(out) }
        }
        ...
        if (act === 'place') {
          ...
          // Placed while inside a block: it becomes a part of that block. A node
          // put down on a block's own canvas and landing on the main one is the
          // canvas lying about what it is a canvas of.
          const into = text(sent.into)
          const host = into === '' ? null : drafts.find((d) => d.block === into && d.expose !== undefined)
          drafts.push({
            id,
      ...
      // Put every node back where the layout would have had it.
      //
      // A dragged position is advisory and this is the other half of that: an
      // arrangement somebody made by hand is theirs to unmake, and without a way
      // back the automatic layout is something you can only lose.
      if (req.path === '/places') {
        places = {}
        loaded = true
        if (store) {
          try { await store.delete(`places:${held.network}`) } catch { /* nothing kept, nothing to clear */ }
        }
        return json(200, { ok: true, why: 'Every node is back where the layout puts it.' })
      }
